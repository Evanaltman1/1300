...
Version: STUDENTS: May 5, 2025
    Stubs: Apr 26, 2025
"""
from collections import Counter  # For convenient collection of word counts
import string
from typing import Self  # For defining punctuation marks.
...
class _Counter(Counter):
    ...
    There is a variable wordlist which is a list of lists passed into _Counter  
    """
    def __init__(self, wordlist=None):
        ...
        # We note that the `wordlist=None` syntax above means that `wordlist` 
        # will be None if the call was _Counter(), i.e., no wordlist was given.
        #
        # In your implementation, you'll want to use the superclass's methods
        # for initializing the word counts (since someone already wrote that code!). 
        # But you need to write your own code to set the running text list.
        self.count = Counter(wordlist)
        if wordlist == None:
            self.wordlist = []
        else:
    ...
    def update(self, other: Self):
        ...
        # You'll want to call a superclass method to update the counts
        # (since somebody already wrote that code!).
        # But you need to write your own code to update the running text list.
      
        if other.wordlist and type(other)==type(self):
            for each in other.wordlist:
                self.wordlist.append(each) 
                self.count.update(each)
    ...
    def __str__(self):
        ...
        if len(self.wordlist)==0:
            return "[]"
        return ', '.join(f'_Counter({word})' for word in self.wordlist) + "]"
